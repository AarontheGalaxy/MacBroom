...
**MacBroom** is an interactive terminal disk cleaner for macOS, written entirely in Bash. It scans 26 categories of junk in parallel, shows you exactly what it found, lets you choose precisely what to delete — and can never touch anything outside a hard-coded list of safe directories.
...
9. [All 26 Cleaning Modules](#all-26-cleaning-modules)
...
- **Hard-coded allowlist** — `mb_safe_rm()` refuses to delete anything outside an explicit list of safe directories; a bug in a module *cannot* delete your home folder or system files
...
MacBroom has five screens. You move between them automatically as you work.

---

### The Scanning Screen

This is the first thing you see every time MacBroom starts. It scans all 26 categories simultaneously (in parallel), so the total wait is only as long as the slowest scan — usually a few seconds.
...
| `—` | Size not yet scanned (see Large Old Files) |
...
**The info bar** (second row from bottom): Shows the safety badge in color (`[S]` green / `[M]` yellow / `[R]` red), followed by a one-line description of the highlighted module, and a scroll position like `(1-10/26)` when the list is taller than your terminal. You always know the safety level of the highlighted row without looking at the left column.
...
**What it cleans:** Log files and crash reports in `~/Library/Logs` that are older than `MB_LOG_AGE_DAYS` days (default: 7). Recent logs are always preserved.
...
1. **DNS cache flush** — `dscacheutil -flushcache` and `killall -HUP mDNSResponder`. Clears stale DNS entries that can cause websites to not resolve correctly.
2. **Font cache rebuild** — `atsutil databases -remove`. Forces macOS to rebuild the font database, fixing font rendering issues. Takes effect after logout.
3. **APFS local snapshot purge** — Deletes local Time Machine snapshots via `tmutil deletelocalsnapshots`. Before running, Dry-Run mode shows exactly how many snapshots exist. During the actual clean, MacBroom prints the snapshot count and a "cannot be undone" warning before proceeding.
4. **Launch Services reset** — Re-registers all apps, fixing broken "Open With" menus and stale file associations.

**Note:** Because this module runs system commands rather than deleting individual files, the Preview screen shows nothing to select. Selecting it and pressing `Enter` runs all four tasks automatically.
...
**System bundle exclusions:** macOS creates many directories for background daemons and system services that have no `.app` in `/Applications`. MacBroom automatically skips these to prevent false positives:
...
**Why it shows `—` on the Main screen:** This module uses `find` to walk potentially large directories, which takes too long to run at startup alongside 25 other scans. The actual scan runs the moment you open the Preview screen (`→`). When you press `←` to return, the real size replaces `—` and the module participates correctly in the size ranking.
...
**What it cleans:** Locally cached email attachments in `~/Library/Mail` that are older than `MB_MAIL_AGE_DAYS` days (default: 90).
...
| `MB_APP_LEFTOVERS_QUERY` | *(empty)* | If set, the App Leftovers module searches only for files matching this app name |
...
Everything else — your home directory root, system directories, shell config files, SSH keys, application binaries — is completely unreachable.
...
### Safeguard 3 — Risky Categories Excluded from Bulk Selection

Categories rated `[R]` (Trash, iMessage Attachments, iOS Backups, AI Models) are excluded from the `A` key on the Main screen. They must be selected individually, manually, every single time — no shortcuts.

### Safeguard 4 — Symlink Target Validation
...
### Safeguard 6 — Zero Network Access
...
### Safeguard 8 — Immutable Flag Handling
...
2. `mb_safe_rm()` (in `lib/core.sh`) first checks `MB_PROTECTED_PATHS` — refuses and returns 1 if matched
3. It then iterates `MB_SAFE_PREFIXES` — refuses and logs if no prefix matches
4. If the path is a symlink, it recursively calls `mb_is_safe_path()` on the resolved target
5. Only if all checks pass does it call `rm -rf -- "$target"` (the `--` prevents path-as-flag injection)
6. The deletion is logged unconditionally after success
...
├── lib/
│   ├── core.sh                   # mb_safe_rm, path allowlist, size formatting, logging
│   ├── disk.sh                   # APFS disk stats: mb_disk_used_kb, mb_disk_total_kb
│   ├── ui.sh                     # All five TUI screens, key reader, spinner, resize handler
...
| --------- | ------------- |
| `test_core.bats` | `mb_is_safe_path` allowlist enforcement, protected path blocking, trailing slash normalization, symlink target validation (safe link, unsafe link, link to `.ssh`), `mb_safe_rm` delete/refuse/idempotent behavior, `mb_format_bytes` formatting |
| `test_orphaned_files.bats` | `_is_system_bundle` exclusions for all vendor prefixes (`com.apple.*`, `com.google.Keystone`, `com.microsoft.autoupdate`, `com.adobe.acc`, `com.crashlytics.*`, `io.fabric.*`, `com.bugsnag.*`, `com.sentry.*`); user apps that must NOT be excluded; `_is_likely_bundle_dir` pattern matching |
| `test_maintenance.bats` | `maintenance_scan` return value; dry-run exit codes; output containing "Flush DNS", "Font Cache", "Launch Services"; "none found" message with no snapshots; snapshot count display with 2 snapshots; irreversible warning when snapshots exist; `result_file` writing |
...
| Array | Purpose |
| ----- | ------- |
| `MB_MOD_NAMES` | Internal ID used by CLI commands (e.g. `browser`) |
| `MB_MOD_LABELS` | Display name shown in the TUI (e.g. `"Browser Caches"`) |
...
This is expected. The module uses `find` across potentially large directories, which takes too long to run alongside 25 other scans at startup. Press `→` to open its Preview screen — the scan runs while the spinner plays. When you press `←` to return, the real size replaces `—` and the size bar appears.
...
- Go module: requires `go` in `$PATH`
- Flutter/Dart: requires `flutter` in `$PATH`
...
MacBroom only deletes paths on its allowlist. If a module finds files in a path that is not on the list, those files are skipped and logged to `~/Library/Logs/MacBroom/errors.log`. Paths that appear to be missing — check `errors.log` for the reason.
